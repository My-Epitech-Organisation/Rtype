...
class IComponentSerializer {
 public:
    ...
    auto operator=(const IComponentSerializer&) -> IComponentSerializer& = default;
     ...
     */
    [[nodiscard]] virtual auto serialize(Entity entity, std::reference_wrapper<Registry> registry) const -> std::string = 0;
...
class Serializer {
 public:
    explicit Serializer(std::reference_wrapper<Registry> reg)
        : registry(reg) {}
         ...
         * @return true if successful
         */
        auto loadFromFile(const std::string& filename, bool clearExisting = true) -> bool;
         ...
         */
        auto deserialize(const std::string& data, bool clearExisting = true) -> bool;
...
class ComponentSerializer : public IComponentSerializer {
 public:
    ...
    [[nodiscard]] auto serialize(Entity entity, std::reference_wrapper<Registry> registry) const -> std::string override;
...
namespace ECS {
template<typename T>
auto ComponentSerializer<T>::serialize(Entity entity, std::reference_wrapper<Registry> registry) const -> std::string {
