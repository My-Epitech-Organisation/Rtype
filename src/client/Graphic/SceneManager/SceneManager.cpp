...
void SceneManager::_applySceneChange() {
    if (this->_nextScene.has_value()) {
        ...
        LOG_DEBUG_CAT(::rtype::LogCategory::UI, "[SceneManager] Applying scene change from "
                  << static_cast<int>(this->_currentScene) << " to "
                  << static_cast<int>(scene));
        this->_currentScene = scene;
        this->_activeScene = this->_sceneList[this->_currentScene]();
        LOG_DEBUG_CAT(::rtype::LogCategory::UI, "[SceneManager] Scene change applied successfully");
...
void SceneManager::setCurrentScene(const Scene scene) {
    ...
    LOG_DEBUG_CAT(::rtype::LogCategory::UI, "[SceneManager] Scene change requested to: "
              << static_cast<int>(scene));
