...
RTypeClient::Graphic::Graphic()
    : _window(nullptr), _renderer(nullptr), _isRunning(true) {
    if (SDL_Init(SDL_INIT_VIDEO) < 0) {
        std::cerr << "SDL_Init Error: " << SDL_GetError() << std::endl;
        return;
    }

    if (SDL_CreateWindowAndRenderer("R-Type Client", 800, 600, 0, &_window,
                                    &_renderer) < 0) {
    ...
    this->_playerTexture =
        loadTextureFromMemory(r_typesheet30a_bmp, r_typesheet30a_bmp_len);
...
SDL_Texture* RTypeClient::Graphic::loadTextureFromMemory(
    ...
    if (!surface) {
        std::cerr << "Erreur chargement Surface: " << SDL_GetError()
                  << std::endl;
        return nullptr;
    }
...
RTypeClient::Graphic::~Graphic() {
    if (_playerTexture) SDL_DestroyTexture(_playerTexture);
