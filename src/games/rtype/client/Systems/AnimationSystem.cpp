...
void rtype::games::rtype::client::AnimationSystem::update(ECS::Registry &registry, float dt)
{
    registry.view<Animation, TextureRect>().each([dt](auto _, Animation &anim, TextureRect &texRect) {
        anim.elapsedTime += dt;
        if (anim.elapsedTime >= anim.frameDuration) {
            ...
            texRect.rect.position.x = (anim.currentFrame - 1) * texRect.rect.size.x;
        }
    });
}
