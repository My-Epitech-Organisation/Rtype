...
void SpawnerSystem::update(ECS::Registry& registry, float deltaTime) {
    ...
    if (_config.maxWaves > 0 &&
        ...
        aliveEnemies == 0) {
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[SpawnerSystem] Wave "
        ...
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[SpawnerSystem] Starting wave " << _currentWave);
    }

    if (_spawnTimer >= _nextSpawnTime &&
        _enemiesSpawnedThisWave < _config.enemiesPerWave) {
        if (_enemyCount >= _config.maxEnemies) {
            LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[SpawnerSystem] Cannot spawn: enemyCount="
                      << _enemyCount
                      << " >= maxEnemies=" << _config.maxEnemies);
        } else {
            spawnBydosSlave(registry);
            _enemiesSpawnedThisWave++;
            LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[SpawnerSystem] Enemy spawned. Total this wave: "
                      << _enemiesSpawnedThisWave << "/"
