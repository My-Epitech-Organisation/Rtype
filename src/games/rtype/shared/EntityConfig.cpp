...
AIBehavior stringToBehavior(const std::string& str) {
    if (str == "move_left" || str == "MoveLeft")
        return AIBehavior::MoveLeft;
    if (str == "sine_wave" || str == "SineWave")
        return AIBehavior::SineWave;
    if (str == "chase" || str == "Chase")
        return AIBehavior::Chase;
    if (str == "patrol" || str == "Patrol")
...
bool EntityConfigRegistry::loadFromDirectory(const std::string& configDir) {
    ...
    // Load entity configuration files
    ...
    // Load level files from levels subdirectory
        ...
        if (auto* enemies = tbl["enemy"].as_array()) {
            for (const auto& elem : *enemies) {
                if (auto* enemyTbl = elem.as_table()) {
                    ...
                    config.spriteSheet = (*enemyTbl)["sprite_sheet"].value_or("");

                    // Stats
                    config.health = (*enemyTbl)["health"].value_or(100);
                    config.damage = (*enemyTbl)["damage"].value_or(10);
                    config.scoreValue = (*enemyTbl)["score_value"].value_or(100);
                    ...
                    // Hitbox
                    config.hitboxWidth = (*enemyTbl)["hitbox_width"].value_or(32.0F);
                    config.hitboxHeight = (*enemyTbl)["hitbox_height"].value_or(32.0F);

                    // Shooting
                    config.canShoot = (*enemyTbl)["can_shoot"].value_or(false);
                    config.fireRate = (*enemyTbl)["fire_rate"].value_or(1.0F);
                    config.projectileType = (*enemyTbl)["projectile_type"].value_or("");
        ...
        if (auto* projectiles = tbl["projectile"].as_array()) {
            for (const auto& elem : *projectiles) {
                if (auto* projTbl = elem.as_table()) {
                    ...
                    config.spriteSheet = (*projTbl)["sprite_sheet"].value_or("");
                    ...
                    config.hitboxWidth = (*projTbl)["hitbox_width"].value_or(8.0F);
                    config.hitboxHeight = (*projTbl)["hitbox_height"].value_or(4.0F);
        ...
        if (auto* players = tbl["player"].as_array()) {
            for (const auto& elem : *players) {
                if (auto* playerTbl = elem.as_table()) {
                    ...
                    config.spriteSheet = (*playerTbl)["sprite_sheet"].value_or("");
                    ...
                    config.hitboxWidth = (*playerTbl)["hitbox_width"].value_or(32.0F);
                    config.hitboxHeight = (*playerTbl)["hitbox_height"].value_or(16.0F);

                    config.defaultProjectile =
                        (*playerTbl)["default_projectile"].value_or("basic_bullet");
        ...
        if (auto* powerups = tbl["powerup"].as_array()) {
            for (const auto& elem : *powerups) {
                if (auto* puTbl = elem.as_table()) {
                    ...
                    config.effect = stringToEffect((*puTbl)["effect"].value_or("health"));
                    config.duration = (*puTbl)["duration"].value_or(0.0F);
                    config.value = (*puTbl)["value"].value_or(25);

                    config.hitboxWidth = (*puTbl)["hitbox_width"].value_or(16.0F);
                    config.hitboxHeight = (*puTbl)["hitbox_height"].value_or(16.0F);
                    ...
                    if (auto* spawns = (*waveTbl)["spawn"].as_array()) {
                        for (const auto& spawnElem : *spawns) {
                            if (auto* spawnTbl = spawnElem.as_table()) {
                                ...
                                spawn.enemyId = (*spawnTbl)["enemy"].value_or("");
                                spawn.x = (*spawnTbl)["x"].value_or(800.0F);
                                spawn.y = (*spawnTbl)["y"].value_or(300.0F);
                                spawn.delay = (*spawnTbl)["delay"].value_or(0.0F);
...
const ProjectileConfig* EntityConfigRegistry::getProjectile(const std::string& id) const {
    auto it = m_projectiles.find(id);
    return (it != m_projectiles.end()) ? &it->second : nullptr;
}

const PlayerConfig* EntityConfigRegistry::getPlayer(const std::string& id) const {
    auto it = m_players.find(id);
    return (it != m_players.end()) ? &it->second : nullptr;
}

const PowerUpConfig* EntityConfigRegistry::getPowerUp(const std::string& id) const {
