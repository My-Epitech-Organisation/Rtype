 ...
 *
 * Player IDs range from 1 to MAX_PLAYER_COUNT, with each ID corresponding to a
 * specific sprite color in the player_vessel sprite sheet.
 */
struct PlayerIdComponent {
    uint32_t playerId =
        0;  ///< Player ID (1-MAX_PLAYER_COUNT), 0 means unassigned
    ...
    [[nodiscard]] bool isValid() const noexcept {
        return playerId >= 1 &&
               playerId <= ::rtype::game::config::MAX_PLAYER_COUNT;
    }
};
