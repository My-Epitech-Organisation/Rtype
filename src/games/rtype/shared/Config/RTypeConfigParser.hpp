...
class RTypeConfigParser {
   public:
     ...
     * @return Loaded configuration, or nullopt on failure
     */
    [[nodiscard]] std::optional<RTypeGameConfig> loadFromString(const std::string& content);
     ...
     */
    bool saveToFile(const RTypeGameConfig& config, const std::filesystem::path& filepath);
     ...
     */
    [[nodiscard]] const rtype::config::ParseResult& getLastResult() const noexcept {
     ...
     */
    [[nodiscard]] const std::vector<rtype::config::ParseError>& getLastErrors() const noexcept {
