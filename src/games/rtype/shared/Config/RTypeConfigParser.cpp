...
std::optional<RTypeGameConfig> RTypeConfigParser::loadFromString(const std::string& content) {
...
bool RTypeConfigParser::saveToFile(const RTypeGameConfig& config,
    ...
    if (!file.is_open()) {
        _parser.reportError({"file", "", "Cannot create file: " + filepath.string()});
        return false;
    }
    ...
    if (!file) {
        _parser.reportError({"file", "", "Failed to write to file: " + filepath.string()});
    ...
    try {
        ...
        _parser.reportError({"file", "", std::string("Failed to save file: ") + e.what()});
        std::filesystem::remove(tempPath);
        return false;
    }
...
std::string RTypeConfigParser::serializeToString(const RTypeGameConfig& config) {
    return serializeToToml(config);
}

void RTypeConfigParser::setErrorCallback(ErrorCallback callback) {
    _parser.setErrorCallback([callback](const rtype::config::ParseError& error) {
...
RTypeGameConfig RTypeConfigParser::parseFromTable(const toml::table& table) {
    ...
    // Video section
    config.video.width = _parser.getValue<uint32_t>(
        table, "video", "width", defaults.video.width);
    config.video.height = _parser.getValue<uint32_t>(
        table, "video", "height", defaults.video.height);
    config.video.fullscreen = _parser.getValue<bool>(
        table, "video", "fullscreen", defaults.video.fullscreen);
    config.video.vsync = _parser.getValue<bool>(
        table, "video", "vsync", defaults.video.vsync);
    config.video.maxFps = _parser.getValue<uint32_t>(
        table, "video", "maxFps", defaults.video.maxFps);
    config.video.uiScale = static_cast<float>(_parser.getValue<double>(
        table, "video", "uiScale", defaults.video.uiScale));

    // Parse resolution string if present (e.g., "1280x720")
        ...
        if (auto res = (*sec)["resolution"].value<std::string>()) {
            ...
            if (xPos != std::string::npos) {
                try {
                    ...
                    _parser.reportError({"video", "resolution", "Invalid resolution format"});
                }
            }
        }
    }
    ...
    config.audio.muted = _parser.getValue<bool>(
        table, "audio", "muted", defaults.audio.muted);
    ...
    config.network.connectionTimeout = _parser.getValue<uint32_t>(
        table, "network", "connectionTimeout", defaults.network.connectionTimeout);
    ...
    config.server.mapName = _parser.getString(
        table, "server", "mapName", defaults.server.mapName);
    ...
    config.gameplay.enemySpeedMultiplier = static_cast<float>(_parser.getValue<double>(
        table, "gameplay", "enemySpeedMultiplier", defaults.gameplay.enemySpeedMultiplier));
    config.gameplay.friendlyFire = _parser.getValue<bool>(
        table, "gameplay", "friendlyFire", defaults.gameplay.friendlyFire);

    // Input section
    config.input.moveUp = _parser.getString(
        table, "input", "moveUp", defaults.input.moveUp);
    config.input.moveDown = _parser.getString(
        table, "input", "moveDown", defaults.input.moveDown);
    config.input.moveLeft = _parser.getString(
        table, "input", "moveLeft", defaults.input.moveLeft);
    config.input.moveRight = _parser.getString(
        table, "input", "moveRight", defaults.input.moveRight);
    config.input.fire = _parser.getString(
        table, "input", "fire", defaults.input.fire);
    config.input.pause = _parser.getString(
        table, "input", "pause", defaults.input.pause);
    ...
    // Paths section
    config.paths.assetsPath = _parser.getString(
        table, "paths", "assetsPath", defaults.paths.assetsPath);
    config.paths.savesPath = _parser.getString(
        table, "paths", "savesPath", defaults.paths.savesPath);
    config.paths.logsPath = _parser.getString(
        table, "paths", "logsPath", defaults.paths.logsPath);
    config.paths.configPath = _parser.getString(
        table, "paths", "configPath", defaults.paths.configPath);
