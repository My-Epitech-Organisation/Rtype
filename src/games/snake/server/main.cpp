...
using rtype::games::snake::server::SnakeGameEngine;
...
static ECS::Entity spawnPlayer(std::shared_ptr<ECS::Registry> reg,
                               uint32_t playerId, int startX, int startY) {
    auto head = reg->spawnEntity();
    reg->emplaceComponent<SnakeHeadComponent>(
        head, SnakeHeadComponent{.playerId = playerId});
    reg->emplaceComponent<PositionComponent>(
        head, PositionComponent{.gridX = startX, .gridY = startY});
    reg->emplaceComponent<VelocityComponent>(
        head, VelocityComponent{.dirX = 1, .dirY = 0});
    reg->emplaceComponent<PlayerInputComponent>(
...
int main(int argc, char** argv) {
    ...
    if (!server.start(port)) {
        std::cerr << "Failed to start NetworkServer on port " << port
                  << std::endl;
        return 1;
    }

    std::cout << "Snake server listening on port " << port << " (local testing)"
              << std::endl;
    std::cout << "Start the Snake client (local) with: ./snake_game"
              << std::endl;
    ...
    auto checkAndStartGame = [&]() {
        ...
        std::cout << "All players ready - starting in " << countdown << "s..."
                  << std::endl;
        server.broadcastGameStart(countdown);
        std::this_thread::sleep_for(
            std::chrono::milliseconds(static_cast<int>(countdown * 1000.0f)));
        ...
        for (auto& p : players) {
            ...
            players[p.first] = ent;
            server.spawnEntity(
                ent.id, rtype::server::NetworkServer::EntityType::Player, 0,
        ...
        server.updateGameState(
            rtype::server::NetworkServer::GameState::Running);
        gameStarted = true;
    ...
    server.onClientDisconnected(
        [&](uint32_t userId, rtype::network::DisconnectReason /*reason*/) {
            ...
            if (gameStarted && players.size() < 2) {
                std::cout << "Not enough players - returning to lobby"
                          << std::endl;
                engine->stopGame();
                server.updateGameState(
                    rtype::server::NetworkServer::GameState::Lobby);
                gameStarted = false;
            }
        });
    ...
    server.onClientReady([&](uint32_t userId, bool isReady) {
        ...
        std::cout << "Client " << userId << (isReady ? " ready" : " not ready")
                  << std::endl;
        checkAndStartGame();
    });
    ...
    server.onClientInput([&](uint32_t userId, uint8_t input) {
        ...
        if (input & rtype::network::InputMask::kUp)
            inputComp.nextDirection = Direction::UP;
        else if (input & rtype::network::InputMask::kDown)
            inputComp.nextDirection = Direction::DOWN;
        else if (input & rtype::network::InputMask::kLeft)
            inputComp.nextDirection = Direction::LEFT;
        else if (input & rtype::network::InputMask::kRight)
            inputComp.nextDirection = Direction::RIGHT;
    });
    ...
    while (running) {
        ...
        if (gameStarted) {
            ...
            if (gameState.isGameOver) {
                std::cout << "Game over! Final score: " << gameState.score
                          << std::endl;
