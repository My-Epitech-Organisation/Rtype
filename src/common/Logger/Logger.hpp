...
class Logger {
 public:
     ...
     * @param logger Pointer to custom logger instance (nullptr to reset)
     */
...
protected:
 ...
 private:
    static inline Logger* _customInstance{nullptr};
    ...
    void log(LogLevel level, const std::string& msg) {
        ...
        const std::string formattedMsg = std::format("[{}] [{}] {}",
            Timestamp::now(), toString(level), msg);

        std::ostream& consoleStream = (level >= LogLevel::Warning) ? std::cerr : std::cout;
