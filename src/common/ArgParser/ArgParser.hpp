...
public:
  ...
  parse(const std::vector<std::string_view> &args) const {
...
private:
   ...
   */
  [[nodiscard]] ParseResult
  parseOption(const std::string &key,
  ...
  void printPositionalArgs() const {
    ...
    for (const auto &posArg : _positionalArgs) {
      std::cout << "  " << std::left
                << std::setw(static_cast<int>(maxWidth + 4)) << posArg.name
                << posArg.description;
      if (!posArg.required) {
        std::cout << " (optional)";
      }
  ...
  std::unordered_map<
      ...
      _valueHandlers;
  std::unordered_map<std::string, std::shared_ptr<std::function<ParseResult()>>>
      _flagHandlers;
