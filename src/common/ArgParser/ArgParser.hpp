 ...
 * - For flags: return ParseResult::Exit for --help, ParseResult::Success to continue
 * - For options: return ParseResult::Error on validation failure, ParseResult::Success otherwise
 * - For positional: return ParseResult::Error on validation failure, ParseResult::Success otherwise
 */
class ArgParser {
public:
    /**
     ...
     * @param description Description for help message
     * @param handler Function to call with the argument value (returns ParseResult)
     * @return Reference to this parser for chaining
     */
    ArgParser& option(std::string_view shortOpt, std::string_view longOpt,
        ...
        _options.push_back({std::string(shortOpt), std::string(longOpt),
                           std::string(description), true, std::string(argName)});
        auto sharedHandler = std::make_shared<std::function<ParseResult(std::string_view)>>(std::move(handler));
...
private:
    ...
    [[nodiscard]] ParseResult processPositionalArgs(
        ...
        for (std::size_t i = 0; i < _positionalArgs.size(); ++i) {
            if (i < positionalValues.size()) {
                ...
                LOG_ERROR(std::format("Missing required argument: {}", _positionalArgs[i].name));
                printUsage();
                return ParseResult::Error;
            }
        }
        if (positionalValues.size() > _positionalArgs.size() && !_positionalArgs.empty()) {
            LOG_WARNING(std::format("Extra positional arguments ignored (got {}, expected {})",
    ...
    [[nodiscard]] bool isDuplicate(std::string_view shortOpt, std::string_view longOpt) const {
        ...
        return _flagHandlers.contains(shortKey) || _flagHandlers.contains(longKey) ||
               _valueHandlers.contains(shortKey) || _valueHandlers.contains(longKey);
    }

    std::string _programName = "program";
    std::vector<Option> _options;
    std::vector<PositionalArg> _positionalArgs;
    std::unordered_map<std::string, std::shared_ptr<std::function<ParseResult(std::string_view)>>> _valueHandlers;
    std::unordered_map<std::string, std::shared_ptr<std::function<ParseResult()>>> _flagHandlers;
