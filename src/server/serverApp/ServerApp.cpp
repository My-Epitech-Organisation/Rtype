...
ServerApp::ServerApp(std::unique_ptr<IGameConfig> gameConfig,
    ...
    if (_gameConfig && _gameConfig->isInitialized()) {
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Configured from game: " << _gameConfig->getGameId());
    } else {
        LOG_WARNING_CAT(::rtype::LogCategory::GameEngine, "[Server] Game config not initialized, using defaults");
    }
}
...
bool ServerApp::run() {
    if (!initialize()) {
        LOG_ERROR_CAT(::rtype::LogCategory::GameEngine, "[Server] Failed to initialize server");
        return false;
    }
    logStartupInfo();
...
void ServerApp::logStartupInfo() const noexcept {
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Starting on port " << _port);
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Max players: " << _clientManager.getMaxPlayers());
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Tick rate: " << _tickRate << " Hz");
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] State: Waiting for players (need "
             << MIN_PLAYERS_TO_START << " ready to start)");
    LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Client timeout: " << _clientTimeoutSeconds << "s");

    if (_gameConfig && _gameConfig->isInitialized()) {
        const auto gameplay = _gameConfig->getGameplaySettings();
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Game: " << _gameConfig->getGameId());
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Difficulty: " << gameplay.difficulty);
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Starting lives: " << gameplay.startingLives);
    }
}

bool ServerApp::reloadConfiguration() {
    if (!_gameConfig || !_gameConfig->isInitialized()) {
        LOG_WARNING_CAT(::rtype::LogCategory::GameEngine, "[Server] Cannot reload - game config not initialized");
        return false;
    }

    if (!_gameConfig->reloadConfiguration()) {
        LOG_ERROR_CAT(::rtype::LogCategory::GameEngine, "[Server] Configuration reload failed");
        return false;
    }

    const auto gameplay = _gameConfig->getGameplaySettings();
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Configuration reloaded:");
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server]   Difficulty: " << gameplay.difficulty);
    ...
    if (serverSettings.port != _port) {
        LOG_WARNING_CAT(::rtype::LogCategory::GameEngine, "[Server] Port change requires restart (current: "
...
bool ServerApp::initialize() {
    ...
    if (!_gameEngine) {
        LOG_ERROR_CAT(::rtype::LogCategory::GameEngine, "[Server] Failed to create game engine");
        return false;
    }
    if (!_gameEngine->initialize()) {
        LOG_ERROR_CAT(::rtype::LogCategory::GameEngine, "[Server] Failed to initialize game engine");
        return false;
    }
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Game engine initialized");
    ...
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Entity spawner created for game: " << gameId);
    ...
    _gameEngine->setEventCallback([this](const engine::GameEvent& event) {
        if (_verbose) {
            LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Game event: type="
    ...
    if (!_networkServer->start(_port)) {
        LOG_ERROR_CAT(::rtype::LogCategory::GameEngine, "[Server] Failed to start network server on port " << _port);
        return false;
    }
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Network server started on port " << _port);

    if (!startNetworkThread()) {
        LOG_ERROR_CAT(::rtype::LogCategory::GameEngine, "[Server] Failed to start network thread");
        return false;
    }

    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Server initialized successfully");
    return true;
}

void ServerApp::shutdown() noexcept {
    if (_hasShutdown.exchange(true, std::memory_order_acq_rel)) {
        LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Shutdown already performed, skipping");
        return;
    }
    ...
    if (_networkServer) {
        _networkServer->stop();
        LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Network server stopped");
    }
    if (_gameEngine && _gameEngine->isRunning()) {
        _gameEngine->shutdown();
        LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Game engine shutdown");
    }

    _clientManager.clearAllClients();
    LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Shutdown complete");
}

void ServerApp::handleClientConnected(std::uint32_t userId) {
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Client connected: userId=" << userId);
    _metrics->totalConnections.fetch_add(1, std::memory_order_relaxed);

    if (_stateManager->isWaiting()) {
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Waiting for client " << userId
                                                << " to signal ready");
    }
    ...
    if (_entitySpawner) {
        ...
        if (!result.success) {
            LOG_ERROR_CAT(::rtype::LogCategory::GameEngine, "[Server] Failed to spawn player for userId=" << userId);
        }
    }
}

void ServerApp::handleClientDisconnected(std::uint32_t userId) {
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Client disconnected: userId=" << userId);
...
void ServerApp::handleStateChange(GameState oldState, GameState newState) {
    switch (newState) {
        case GameState::Playing:
            _score = 0;
            LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] *** GAME STARTED *** ("
        ...
        case GameState::Paused:
            LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Game paused - waiting for players to reconnect");
            break;
        case GameState::WaitingForPlayers:
            if (oldState == GameState::Paused) {
                LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Resuming wait for players");
            } else if (oldState == GameState::GameOver) {
                LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Back to lobby - waiting for players");
            }
            break;
        case GameState::GameOver:
            LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] *** GAME OVER *** Final score=" << _score);
            if (_networkSystem) {
                LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Broadcasting GameOver via NetworkSystem");
                ...
                LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Sending GameOver via NetworkServer");
...
void ServerApp::processIncomingData() noexcept {
    ...
    while (auto packetOpt = _incomingPackets.pop()) {
        ...
        if (clientId == ClientManager::INVALID_CLIENT_ID) {
            ...
            if (clientId == ClientManager::INVALID_CLIENT_ID) {
                LOG_WARNING_CAT(::rtype::LogCategory::GameEngine, "[Server] Rejected connection from "
...
bool ServerApp::startNetworkThread() {
    try {
        ...
        LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Network thread started");
        return true;
    } catch (const std::exception& e) {
        LOG_ERROR_CAT(::rtype::LogCategory::GameEngine, "[Server] Failed to start network thread: " << e.what());
...
void ServerApp::stopNetworkThread() noexcept {
    if (_networkThreadRunning.load(std::memory_order_acquire)) {
        ...
        LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Network thread stopped");
    }
}

void ServerApp::networkThreadFunction() noexcept {
    LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Network thread running");

    while (_networkThreadRunning.load(std::memory_order_acquire)) {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }

    LOG_DEBUG_CAT(::rtype::LogCategory::GameEngine, "[Server] Network thread exiting");
}
...
void ServerApp::checkGameOverCondition() {
    ...
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] All players defeated - ending game");
...
void ServerApp::resetToLobby() {
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Resetting session to lobby");
    ...
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine, "[Server] Reset to lobby complete");
}
