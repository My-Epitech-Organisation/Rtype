...
void GameSession::handleClientConnected(std::uint32_t userId) {
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine,
                 "[GameSession] Client connected: userId=" << userId);

    if (_state == GameState::WaitingForPlayers) {
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine,
                     "[GameSession] Waiting for client " << userId
                                                         << " to signal ready");
    }

    if (!_entitySpawner) {
        LOG_ERROR_CAT(::rtype::LogCategory::GameEngine,
                      "[GameSession] No entity spawner available");
        return;
    }
    ...
    if (result.success) {
        LOG_INFO_CAT(::rtype::LogCategory::GameEngine,
                     "[GameSession] Spawned player for userId="
                         << userId << " networkId=" << result.networkId
                         << " pos=(" << result.x << ", " << result.y << ")");
    } else {
        LOG_ERROR_CAT(
            ::rtype::LogCategory::GameEngine,
            "[GameSession] Failed to spawn player for userId=" << userId);
    }
}

void GameSession::handleClientDisconnected(std::uint32_t userId) {
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine,
                 "[GameSession] Client disconnected: userId=" << userId);
...
void GameSession::transitionToState(GameState newState) {
    ...
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine,
                 "[GameSession] State transition: "
                     << toString(_state) << " -> " << toString(newState));
    ...
    switch (newState) {
        ...
        case GameState::WaitingForPlayers:
            if (oldState == GameState::Paused) {
                LOG_INFO_CAT(::rtype::LogCategory::GameEngine,
                             "[GameSession] Resuming wait for players");
            }
            break;
        case GameState::GameOver:
            LOG_INFO_CAT(
                ::rtype::LogCategory::GameEngine,
                "[GameSession] Game ended - session can be cleaned up");
            break;
    }
...
void GameSession::stopGame() {
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine,
                 "[GameSession] Stopping game session");
    ...
    LOG_INFO_CAT(::rtype::LogCategory::GameEngine,
                 "[GameSession] Game session stopped");
}
