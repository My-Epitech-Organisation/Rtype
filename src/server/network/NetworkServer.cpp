...
void NetworkServer::updateEntityHealth(std::uint32_t id, std::int32_t current,
                                       std::int32_t max) {
    LOG_DEBUG_CAT(::rtype::LogCategory::Network, "[NetworkServer] updateEntityHealth: entityId="
...
void NetworkServer::sendGameOver(std::uint32_t finalScore) {
    ...
    LOG_INFO_CAT(::rtype::LogCategory::Network, "[NetworkServer] GameOver packet broadcasted to all clients");
}
...
void NetworkServer::processIncomingPacket(const network::Buffer& data,
    ...
    if (header.flags & network::Flags::kIsAck) {
        ...
        if (client) {
            LOG_DEBUG_CAT(::rtype::LogCategory::Network, "[NetworkServer] Processing ACK from userId="
    ...
    if (!seqResult) {
        LOG_DEBUG_CAT(::rtype::LogCategory::Network, "[NetworkServer] Sequence validation failed for userId="
...
void NetworkServer::handleDisconnect(const network::Header& header,
    ...
    LOG_INFO_CAT(::rtype::LogCategory::Network, "[NetworkServer] Client requested disconnect userId=" << userId);
