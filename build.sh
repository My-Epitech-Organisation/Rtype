...
if [ "$BUILD_TESTS" = true ]; then
    ...
    cmake --build --preset "$CMAKE_PRESET" --target r-type_client r-type_server rtype-display-sfml rtype-graphic-background-asteroidsSpace rtype-graphic-background-lobby rtype-graphic-background-labs rtype-graphic-background-spatialStation rtype-graphic-background-meteorInside rtype-graphic-music-chillMusic rtype-graphic-music-battleMusic rtype-graphic-music-exploreMusic -- -j$(nproc 2>/dev/null || echo 4)
fi
echo "✓ Build complete"
echo ""
