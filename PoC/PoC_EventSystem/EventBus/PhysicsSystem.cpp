...
namespace PoC {
    ...
    PhysicsSystem::PhysicsSystem(EventBus& eventBus) : _eventBus(eventBus) {
        ...
        for (size_t i = 0; i < entities.size(); ++i) {
            for (size_t j = i + 1; j < entities.size(); ++j) {
                ...
                if (dist < collisionDist) {
                    ...
                    float collisionX = (posA.x + posB.x) / 2.0f;
                    float collisionY = (posA.y + posB.y) / 2.0f;

                    std::cout << "[PhysicsSystem] Collision detected between Entity " 
                    ...
                    // PhysicsSystem doesn't know or care who listens
                    _eventBus.publish(CollisionEvent(entityA, entityB, collisionX, collisionY));
