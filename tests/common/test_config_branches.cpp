...
TEST(RTypeGameConfigBranchTest, ApplyDefaults_ResetsInvalidValuesToDefaults) {
    RTypeGameConfig config = RTypeGameConfig::createDefault();
    RTypeGameConfig defaults = RTypeGameConfig::createDefault();

    // Invalidate many fields
    config.video.width = 0;
    config.video.height = 0;
    config.video.maxFps = 0;
    config.video.uiScale = 0.0F;

    config.audio.masterVolume = -5.0F;
    config.audio.musicVolume = -5.0F;
    config.audio.sfxVolume = -5.0F;
    ...
    EXPECT_EQ(config.video.maxFps, defaults.video.maxFps);
    EXPECT_EQ(config.video.uiScale, defaults.video.uiScale);

    EXPECT_EQ(config.audio.masterVolume, defaults.audio.masterVolume);
    EXPECT_EQ(config.audio.musicVolume, defaults.audio.musicVolume);
    EXPECT_EQ(config.audio.sfxVolume, defaults.audio.sfxVolume);

    EXPECT_EQ(config.network.serverAddress, defaults.network.serverAddress);
    EXPECT_EQ(config.network.serverPort, defaults.network.serverPort);
    EXPECT_EQ(config.network.connectionTimeout, defaults.network.connectionTimeout);
    EXPECT_EQ(config.network.tickrate, defaults.network.tickrate);

    EXPECT_EQ(config.server.port, defaults.server.port);
    EXPECT_EQ(config.server.maxPlayers, defaults.server.maxPlayers);
    EXPECT_EQ(config.server.tickrate, defaults.server.tickrate);

    EXPECT_EQ(config.gameplay.difficulty, defaults.gameplay.difficulty);
    EXPECT_EQ(config.gameplay.startingLives, defaults.gameplay.startingLives);
    EXPECT_EQ(config.gameplay.waves, defaults.gameplay.waves);
    EXPECT_EQ(config.gameplay.playerSpeed, defaults.gameplay.playerSpeed);
    EXPECT_EQ(config.gameplay.enemySpeedMultiplier, defaults.gameplay.enemySpeedMultiplier);

    EXPECT_EQ(config.input.moveUp, defaults.input.moveUp);
    EXPECT_EQ(config.input.moveDown, defaults.input.moveDown);
    EXPECT_EQ(config.input.moveLeft, defaults.input.moveLeft);
    EXPECT_EQ(config.input.moveRight, defaults.input.moveRight);
    EXPECT_EQ(config.input.fire, defaults.input.fire);
    EXPECT_EQ(config.input.mouseSensitivity, defaults.input.mouseSensitivity);
}
