...
/**
 * @brief View for iterating entities with specific components while excluding
 * others.
 *
 * Provides efficient filtering by checking exclusion criteria only for matching
 * entities.
 *
 * Example:
...
class ExcludeView<std::tuple<Includes...>, std::tuple<Excludes...>> {
   ...
   public:
    ...
    ExcludeView(std::reference_wrapper<Registry> reg,
                std::tuple<PoolPtr<Includes>...> inc_pools,
                ExcludeTuple&& exc_pools, size_t smallest_idx)
        : registry(reg),
          _includePools(inc_pools),
          _excludePools(std::forward<ExcludeTuple>(exc_pools)),
