 ...
 *
 * Prefabs allow you to define entity "blueprints" with predefined component
 * sets. This is useful for:
...
class PrefabManager {
   public:
    ...
    explicit PrefabManager(std::reference_wrapper<Registry> reg)
        : _registry(reg) {}
     ...
     * @return Newly created entity
     */
    auto instantiate(const std::string& name, const PrefabFunc& customizer)
        -> Entity;
     ...
     */
    auto instantiateMultiple(const std::string& name, size_t count)
        -> std::vector<Entity>;
